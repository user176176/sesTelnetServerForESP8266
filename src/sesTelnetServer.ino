...
byte telnetRead(WiFiClient client);
byte telnetInput(WiFiClient client, char *cmdLine, byte len);
...
WiFiServer telnetServer = WiFiServer(23); // Server on this port.
#if defined(TELNET_MULTISERVER)
WiFiServer goawayServer = WiFiServer(23); // Additional listener.
#endif
WiFiClient     client;                            // Client connection.
...
byte telnetInput(WiFiClient client, char *cmdLine, byte len)
{
  ...
  while(!done)
  {
    ...
    WiFiClient client2 = goawayServer.available();
    if (client2)
    {
